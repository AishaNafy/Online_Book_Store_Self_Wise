...
public class ItemsDBUtil {
     ...
     */
    public static boolean UpdateItem(String itemNo, String itemName, String description , String price) {
        boolean isSuccess = false;

        try {
            con = DBConnect.getConnection();
            stmt = con.createStatement();
            String sql = "update items set itemName = '" + itemName + "', description = '" + description + "', price = '" + price + "' "
                    + "where itemNo = '" + itemNo + "'";
